...
function createClerkVerifier(): TokenVerifier {
  ...
  return async (token: string) => {
    try {
      ...
      const meta = payload.publicMetadata as
        ...
        | undefined;
      if (!meta?.userId || !UUID_RE.test(meta.userId)) return null;
      if (!meta?.role || !USER_ROLE_SET.has(meta.role)) return null;
 ...
 */
export function createAuthPreHandler(verifyToken?: TokenVerifier) {
  const verify = verifyToken ?? createClerkVerifier();
